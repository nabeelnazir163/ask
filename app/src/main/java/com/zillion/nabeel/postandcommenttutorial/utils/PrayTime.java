package com.zillion.nabeel.postandcommenttutorial.utils;

/**
 * Created by Nabeel on 9/6/2017.
 */
...
// double R = 1.00014 - 0.01671 * [self dcos:g] - 0.00014 * [self dcos:
    ...
    private ArrayList<String> getDatePrayerTimes(int year, int month, int day,
                                                 double m_latitude, double m_longitude, double tZone) {
        this.setLat(m_latitude);
        this.setLng(m_longitude);
        this.setTimeZone(tZone);
        this.setJDate(julianDate(year, month, day));
        double lonDiff = m_longitude / (15.0 * 24.0);
        this.setJDate(this.getJDate() - lonDiff);
        return computeDayTimes();
    }

    // return prayer times for a given date
    public ArrayList<String> getPrayerTimes(Calendar date, double m_latitude,
                                            double m_longitude, double tZone) {

        int year = date.get(Calendar.YEAR);
        int month = date.get(Calendar.MONTH);
        int day = date.get(Calendar.DATE);

        return getDatePrayerTimes(year, month + 1, day, m_latitude, m_longitude,
                tZone);
    }
